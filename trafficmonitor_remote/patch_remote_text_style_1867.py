...
skin = root / 'TrafficMonitor' / 'SkinFile.cpp'
...
s = skin.read_text(encoding='utf-8-sig')
start = s.find('enum class RemoteVectorIcon')
end = s.find('void CSkinFile::DrawInfo(CDC* pDC, bool show_more_info)', start)
if start < 0 or end < 0:
    raise RuntimeError('remote drawing helper not found')
...
static void DrawRemoteVectorIcon(IDrawCommon& drawer, RemoteVectorIcon icon, CRect rect, COLORREF color)
{
    ...
    case RemoteVectorIcon::Upload:
    {
        ...
        const int top = y + 1;
        const int bottom = y + size - 1;
        drawer.FillRect(CRect(cx, top, cx + stroke, bottom), color);
        drawer.DrawLine(CPoint(cx, top), -(size / 3), color);
        drawer.DrawLine(CPoint(cx, top), size / 3, color);
        drawer.FillRect(CRect(x + 2, bottom - stroke, x + size - 2, bottom), color);
        break;
    }
    case RemoteVectorIcon::Download:
    {
        ...
        const int bottom = y + size - 1;
        drawer.FillRect(CRect(cx, top, cx + stroke, bottom), color);
        drawer.DrawLine(CPoint(cx, bottom - 1), -(size / 3), color);
        drawer.DrawLine(CPoint(cx, bottom - 1), size / 3, color);
        drawer.FillRect(CRect(x + 2, top, x + size - 2, top + stroke), color);
        break;
    }
    case RemoteVectorIcon::Uptime:
    {
        ...
        drawer.DrawRectOutLine(CRect(cx - radius, cy - radius, cx + radius + 1, cy + radius + 1), color, stroke);
        drawer.FillRect(CRect(cx, cy - radius + 2, cx + stroke, cy + 1), color);
        drawer.FillRect(CRect(cx, cy, cx + radius - 1, cy + stroke), color);
        break;
    }
...
static void DrawRemoteMonitorRows(IDrawCommon& drawer, CFont& font, int width, int top, int row_height, COLORREF color)
{
    ...
    // DrawItemsInfo pass. No remote font, no size override, no weight override.
    drawer.SetFont(&font);
    ...
    auto draw_icon_value = [&](int row, int col, RemoteVectorIcon icon, const CString& value)
    {
        ...
        // Icon dimensions follow the same skin text-height used by the first row.
        ...
        if (text_left < cell.right)
        {
            ...
            // Same DrawWindowText implementation, font, DPI scaling and vertical
            // text rectangle height as the original TrafficMonitor items.
    ...
    // Row 2 uses vector symbols so Unicode arrow/clock fallback fonts can no
    // longer make this row look thinner/smaller than TrafficMonitor's first row.
...
skin.write_text(s, encoding='utf-8-sig')
print('TrafficMonitor 1.86.7 remote rows now reuse the exact main skin font/style')
